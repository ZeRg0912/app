...
void LCD_I2C(uint8_t lcd_addr, uint8_t lcd_cols, uint8_t lcd_rows, uint8_t charsize)
{
...
void LCD_begin(void) 
{
	...
	if (lcd_v._rows > 1)
		lcd_v._displayfunction |= LCD_2LINE;

	// for some 1 line displays you can select a 10 pixel high font
	if ((lcd_v._charsize != 0) && (lcd_v._rows == 1))
		lcd_v._displayfunction |= LCD_5x10DOTS;
	...
	LCD_expanderWrite(lcd_v._backlightval);	// reset expanderand turn backlight off (Bit 8 =1)
	_delay_ms(1000);
	...
	LCD_command(LCD_FUNCTIONSET | lcd_v._displayfunction);  
